...
class Visin extends CI_Controller {
    ...
    $output['region_pemasukan']=$this->region_pemasukan($data);
    $output['total_biaya']=$this->total_biaya($data);
    $output['mas_sales']=$this->mas_sales($data);
    ...
    function region_pemasukan($data)
    {
        $result=array();
        foreach($data as $row)
        {
            if(isset($result[$row->Region]) == false)
            {
                ...
                $result[$row->Region]=$total + $row->Total;
            }
        };
        //konversi dalam format tabulasi
        $keys=array_keys($result);
    ...
    function mas_sales($data)
    {
        ...
        foreach($data as $row)
        {
            if(isset($result[$row->Rep]) == false)
            {
                $result[$row->Rep]=$row->Total;
            }else{
                $total=$result[$row->Rep];
                $result[$row->Rep]=$total + $row->Total;
            }
        };
        //sorting data berdasarkan value array secara menurun
        arsort($result);
